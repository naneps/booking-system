...
const { currentBranch, branchList, loading: branchLoading } = storeToRefs(branchStore);

// Handle Switch Branch
const handleBranchChange = (branch: any) => {
  branchStore.setBranch(branch.id);
  // Opsional: Kasih feedback visual
  toast.add({ title: `Switched to ${branch.name}`, color: 'primary', icon: 'i-lucide-check' });
};

// --- MENU & LINKS (Sama seperti sebelumnya) ---
const items = [[
  { label: 'New booking', icon: 'i-lucide-calendar-plus', to: '/bookings/create-new' },
  { label: 'Add walk-in', icon: 'i-lucide-user-plus', to: '/waiting-list/create' },
...
const links = [
  [
    { label: "Dashboard", icon: "i-lucide-house", to: "/", onSelect: () => { open.value = false } },
    { label: "Floor Planner", icon: "i-lucide-layout", to: "/floor-planner", onSelect: () => { open.value = false } },
    {
      label: "Bookings", icon: "i-lucide-calendar", to: "/bookings", type: "trigger",
      children: [
    ...
    { label: "Waiting List", icon: "i-lucide-clock", to: "/waiting-list", onSelect: () => { open.value = false } },
    {
      label: "Master Data", icon: "i-lucide-database", to: "/master", type: "trigger",
      children: [
    ...
    { label: "Staff", icon: "i-lucide-user-check", to: "/staff", onSelect: () => { open.value = false } },
    { label: "Payments", icon: "i-lucide-credit-card", to: "/payments", onSelect: () => { open.value = false } },
    {
      label: "Reports", icon: "i-lucide-pie-chart", to: "/reports", type: "trigger",
      children: [
    ...
    {
      label: "Settings", icon: "i-lucide-settings", to: "/settings", type: "trigger",
      children: [
  ...
  [{ label: "Help & Support", icon: "i-lucide-life-buoy", to: "#", target: "_blank" }],
] satisfies NavigationMenuItem[][];

const groups = computed(() => [{ id: "links", label: "Go to", items: links.flat() }]);
      ...
      <template #header>
        <UDashboardNavbar :ui="{right: 'gap-3'}">
          ...
          <template #leading>
            ...
            <div class="hidden lg:block ml-2 pl-4 border-l border-gray-200 dark:border-gray-800 min-w-[200px]">
                     ...
                     >
                        <div class="flex items-center gap-2 text-left">
                           ...
                           <div class="flex flex-col">
                              <span class="text-[10px] text-gray-500 leading-none uppercase tracking-wider font-semibold">Branch</span>
                              <span class="text-sm font-bold text-gray-900 dark:text-white leading-tight truncate max-w-[140px]">
          ...
          <template #right>
            <UTooltip text="Notifications" :shortcuts="['N']">
              <UButton color="neutral" variant="ghost" square @click="isNotificationsSlideoverOpen = true">
                <UChip color="error" inset><UIcon name="i-lucide-bell" class="size-5 shrink-0" /></UChip>
              </UButton>
            </UTooltip>
