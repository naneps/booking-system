...
export const useBookingStore = defineStore("booking-store", () => {
  ...
  async function fetchBookings(
    params: {q?: string; page: number; per_page: number} & BookingFilter,
  ) {
    loading.value = true;
    error.value = null;
    try {
      const response = await useApi<any>("/api/v1/bookings", {
        method: "GET",
        params: {...params, branch_id: brancHId.value}, // branch_id, from, to, status passed here
      });
  ...
  async function createBooking(payload: BookingForm) {
    ...
    try {
      const response = await useApi<any>("/api/v1/bookings", {
        method: "POST",
        body: {...payload, branch_id: brancHId.value}, // branch_id, start_at, table_ids, etc.
      });
      return response.data;
    } catch (err: any) {
      error.value =
      ...
      loading.value = false;
      throw err; // Re-throw so component can handle UI feedback1
    }
  }
  ...
  async function updateBooking(id: number, payload: Partial<BookingForm>) {
    ...
    try {
      ...
      error.value =
        err.data?.message || err.message || "Failed to update booking";
      throw err;
    } finally {
      loading.value = false;
    }
